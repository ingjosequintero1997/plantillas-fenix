...
using the _norm() function from main.py to find mismatches."""
...
def _norm(s):
    """Normalize from main.py: NFKD, uppercase, alnum+space, join with _"""
    s = str(s).strip()
...
# Build normalized -> DB col map (first match wins)
norm_to_db = {}
...
print("=" * 100)
print("NORMALIZED DB COLUMNS (unique keys):")
print("=" * 100)
for n, col in sorted(norm_to_db.items()):
    print(f"  {n:70s} -> {col}")

print(f"\nTotal unique normalized DB keys: {len(norm_to_db)}")
...
# Track duplicates for "Quien Realizó el Control"
control_count = 0
mismatches = {}
auto_ok = 0
no_match = 0
...
for csv_name, field_type in RAW_FIELDS:
    ...
    if field_type == "FORMULA":
        print(f"  SKIP (FORMULA):  CSV={csv_name!r:60s} -> norm={csv_norm!r}")
        continue
    ...
    if csv_norm in norm_to_db:
        ...
        if csv_name != db_col:
            ...
            print(f"  MISMATCH (norm): CSV={csv_name!r:60s} -> DB={db_col}")
        else:
            auto_ok += 1
    else:
        no_match += 1
        print(f"  NO MATCH:        CSV={csv_name!r:60s} -> norm={csv_norm!r}")

print(f"\nAuto-matched (names differ but norm same): {len(mismatches)}")
print(f"Auto-matched (names identical): {auto_ok}")
print(f"No match at all: {no_match}")

print("\n" + "=" * 100)
print("MISMATCHES WHERE NORM MATCHES BUT NAMES DIFFER:")
print("=" * 100)
for csv_name, db_col in sorted(mismatches.items()):
    print(f"  {csv_name!r:60s} -> {db_col}")

print("\n" + "=" * 100)
print("CSV HEADERS WITH NO DB MATCH:")
print("=" * 100)
for csv_name, field_type in RAW_FIELDS:
    csv_norm = _norm(csv_name)
    if field_type == "FORMULA":
        continue
    if csv_norm not in norm_to_db:
        print(f"  CSV={csv_name!r:60s} norm={csv_norm!r}")
