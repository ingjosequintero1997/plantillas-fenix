...
// Descarga el reporte de errores en EXCEL con celdas en rojo y comentarios de correccion.
export async function descargarReporteErroresExcel(cargueId, filename = 'reporte_errores.xlsx') {
  const resp = await fetch(`${API_BASE}/cargues/${cargueId}/reporte-errores-excel`, {
    method: 'POST',
    headers: authHeaders(),
  })
  if (!resp.ok) {
