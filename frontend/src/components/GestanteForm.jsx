...
const SECCIONES = [
  ...
  },
  {
    ...
    icono: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z',
    fields: [
      { key: 'HA_SIDO_VICTIMA_DE_VIOLENCIA_FISICA_O_PSICOLOGICA', label: 'Victima violencia fisica/psicologica', type: 'select', options: ['Si', 'No'] },
      { key: 'HA_SIDO_VICTIMA_DE_ABUSO_SEXUAL', label: 'Victima abuso sexual', type: 'select', options: ['Si', 'No'] },
      { key: 'SE_IDENTIFICAN_CAUSALES_PARA_IVE', label: 'Causales para IVE', type: 'select', options: ['Si', 'No'] },
    ],
  },
  {
    titulo: 'Ecografias',
    ...
    fields: [
      { key: 'PRIMERA_ECOGRAFIA_OBSTETRICA_10_13', label: '1ra ecografia obstetrica (10-13 sem)', type: 'date' },
      { key: 'SEGUNDA_ECOGRAFIA', label: '2da ecografia', type: 'date' },
      { key: 'ECOGRAFIA_OBSTETRICA_PARA_LA_DETECCION_DE_ANOMALIAS_ESTRUCTURALES_18_23', label: 'Ecografia anomalias estructurales (18-23 sem)', type: 'date' },
      { key: 'OTRAS_ECOGRAFIAS', label: 'Otras ecografias', type: 'text' },
    ],
  },
  {
    titulo: 'Suplementacion y tamizajes adicionales',
    icono: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z',
    fields: [
      { key: 'FECHA_SUMINISTRO_ACIDO_FOLICO', label: 'Fecha suministro acido folico', type: 'date' },
      { key: 'FECHA_SUMINISTRO_CALCIO_SEMANA_14', label: 'Fecha suministro calcio (sem 14)', type: 'date' },
      { key: 'FECHA_SUMINISTRO_HIERRO', label: 'Fecha suministro hierro', type: 'date' },
      { key: 'FECHA_DESPARASITACION_ANTIHELMINTICA_II_Y_III_TRIMESTRE_ALBENDAZO_400_MG_DOSIS_UNICA', label: 'Fecha desparasitacion II/III trimestre', type: 'date' },
      { key: 'INFORMACION_EN_SALUD', label: 'Informacion en salud', type: 'text' },
      { key: 'FECHA_DE_TAMIZAJE_PARA_CA_CUELLO_UTERINO', label: 'Fecha tamizaje CA cuello uterino', type: 'date' },
      { key: 'FECHA_REALIZACION_HEMOCLASIFICACION', label: 'Fecha hemoclasificacion', type: 'date' },
      { key: 'FECHA_REALIZACION_PRUEBA_TAMIZAJE_ESTREPTOCOCO_GRUPO_B', label: 'Fecha tamizaje Estreptococo Grupo B', type: 'date' },
      { key: 'FECHA_REALIZACION_PRUEBA_TOLERANCIA_ORAL_GLUCOSA', label: 'Fecha prueba tolerancia oral glucosa', type: 'date' },
    ],
  },
  {
    titulo: 'VIH/Sifilis - Trimestres y confirmacion',
    icono: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z',
    fields: [
      ...
      { key: 'TOMA_SEGUNDA_PRUEBA_VIH', label: 'Fecha 2da prueba VIH (seguimiento)', type: 'date' },
      { key: 'TRIMESTRE_TOMA_SEGUNDA_PRUEBA_VIH', label: 'Trimestre 2da prueba VIH seguimiento', type: 'text' },
      { key: 'PRUEBA_CONFIRMATORIA_SEGUN_ALGORITMO', label: 'Fecha prueba confirmatoria', type: 'date' },
      { key: 'TRIMESTRE_PRUEBA_CONFIRMATORIA_SEGUN_ALGORITMO', label: 'Trimestre prueba confirmatoria', type: 'text' },
      { key: 'FTA_ABS', label: 'Fecha FTA-ABS', type: 'date' },
      { key: 'TRIMESTRE_FTA_ABS', label: 'Trimestre FTA-ABS', type: 'text' },
  ...
  {
    titulo: 'Controles 6-10',
    icono: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z',
    fields: [
      { key: 'FECHA_6TO_CONTROL', label: 'Fecha 6to control', type: 'date' },
      { key: 'QUIEN_REALIZO_EL_CONTROL_6', label: 'Quien realizo 6to control', type: 'select', options: ['Medico Ginecologia', 'Aux de Enfermeria', 'Enfermera (o)', 'Control Tradicional (obligatorio)'] },
      { key: 'FECHA_7MO_CONTROL', label: 'Fecha 7mo control', type: 'date' },
      { key: 'QUIEN_REALIZO_EL_CONTROL_7', label: 'Quien realizo 7mo control', type: 'select', options: ['Medico Ginecologia', 'Aux de Enfermeria', 'Enfermera (o)', 'Control Tradicional (obligatorio)'] },
      { key: 'FECHA_8VO_CONTROL', label: 'Fecha 8vo control', type: 'date' },
      { key: 'QUIEN_REALIZO_EL_CONTROL_8', label: 'Quien realizo 8vo control', type: 'select', options: ['Medico Ginecologia', 'Aux de Enfermeria', 'Enfermera (o)', 'Control Tradicional (obligatorio)'] },
      { key: 'FECHA_9NO_CONTROL', label: 'Fecha 9no control', type: 'date' },
      { key: 'QUIEN_REALIZO_EL_CONTROL_9', label: 'Quien realizo 9no control', type: 'select', options: ['Medico Ginecologia', 'Aux de Enfermeria', 'Enfermera (o)', 'Control Tradicional (obligatorio)'] },
      { key: 'FECHA_OTROS_CONTROLES_PRENATALES', label: 'Fecha otros controles', type: 'date' },
      { key: 'QUIEN_REALIZO_EL_CONTROL_10', label: 'Quien realizo otros controles', type: 'select', options: ['Medico Ginecologia', 'Aux de Enfermeria', 'Enfermera (o)', 'Control Tradicional (obligatorio)'] },
      { key: 'QUIEN_REALIZO_LA_CONSULTA', label: 'Quien realizo la consulta', type: 'text' },
    ],
  },
  {
    titulo: 'Patologias por trimestre',
    icono: 'M13 10V3L4 14h7v7l9-11h-7z',
    fields: [
      { key: 'HIPERTENSION_INDUCIDA_POR_EMBARAZO_1_TRIM', label: 'Hipertension inducida 1er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'HIPERTENSION_INDUCIDA_POR_EMBARAZO_2_TRIM', label: 'Hipertension inducida 2do Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'HIPERTENSION_INDUCIDA_POR_EMBARAZO_3_TRIM', label: 'Hipertension inducida 3er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'SANGRADO_VAGINAL_1_TRIM', label: 'Sangrado vaginal 1er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'SANGRADO_VAGINAL_2_TRIM', label: 'Sangrado vaginal 2do Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'SANGRADO_VAGINAL_3_TRIM', label: 'Sangrado vaginal 3er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'INFECCION_URINARIA_1_TRIM', label: 'Infeccion urinaria 1er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'INFECCION_URINARIA_2_TRIM', label: 'Infeccion urinaria 2do Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'INFECCION_URINARIA_3_TRIM', label: 'Infeccion urinaria 3er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'VIH_1_TRIM', label: 'VIH 1er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'VIH_2_TRIM', label: 'VIH 2do Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'VIH_3_TRIM', label: 'VIH 3er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'SIFILIS_1_TRIM', label: 'Sifilis 1er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'SIFILIS_2_TRIM', label: 'Sifilis 2do Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'SIFILIS_3_TRIM', label: 'Sifilis 3er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'HEPATITIS_B_1_TRIM', label: 'Hepatitis B 1er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'HEPATITIS_B_2_TRIM', label: 'Hepatitis B 2do Trim', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'HEPATITIS_B_3_TRIM', label: 'Hepatitis B 3er Trim', type: 'select', options: ['NA', 'Si', 'No'] },
    ],
  },
  {
    ...
    fields: [
      { key: 'OTRAS_PATOLOGIAS_DESCRIPCION_Y_FECHA', label: 'Otras patologias', type: 'text' },
      { key: 'PRIORIZADA_PARA_SEGUIMIENTO_ESPECIAL', label: 'Priorizada seguimiento especial', type: 'select', options: ['Si', 'No'] },
      { key: 'NOTIFICACION_A_LA_IPS', label: 'Notificacion a la IPS', type: 'select', options: ['Si', 'No'] },
      { key: 'REMISION', label: 'Remision', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'VISITA_DOMICILIARA', label: 'Visita domiciliaria', type: 'select', options: ['Si', 'No'] },
      { key: 'ACOMPA\xD1AMIENTO_DURANTE_CPN_IMAGENES_DX_Y_EXAMENES_DE_LABORATORIO', label: 'Acompa\u00f1amiento CPN/lab', type: 'text' },
      { key: 'CASA_DE_PASO', label: 'Casa de paso', type: 'select', options: ['Si', 'No'] },
      { key: 'APOYO_PARA_TRANSPORTE', label: 'Apoyo para transporte', type: 'select', options: ['Si', 'No'] },
      { key: 'ACTIVACION_DE_RED_DE_APOYO_COMUNITARIA', label: 'Activacion red apoyo comunitaria', type: 'select', options: ['Si', 'No'] },
      { key: 'COORDINACION_DE_ESTRATEGIAS_CON_SDSM', label: 'Coordinacion estrategias SDS', type: 'text' },
      { key: 'OBSERVACIONES_GESTION_RIESGO', label: 'Observaciones gestion riesgo', type: 'textarea' },
    ],
  },
  {
    titulo: 'Eventos criterio y MME',
    icono: 'M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
    fields: [
      { key: 'TIPO_DE_EVENTO_CRITERIO_1', label: 'Evento criterio 1', type: 'text' },
      { key: 'FECHA_CRITERIO_1', label: 'Fecha criterio 1', type: 'date' },
      { key: 'TIPO_DE_EVENTO_CRITERIO_2', label: 'Evento criterio 2', type: 'text' },
      { key: 'FECHA_CRITERIO_2', label: 'Fecha criterio 2', type: 'date' },
      { key: 'TIPO_DE_EVENTO_CRITERIO_3', label: 'Evento criterio 3', type: 'text' },
      { key: 'FECHA_CRITERIO_3', label: 'Fecha criterio 3', type: 'date' },
      { key: 'CAUSA_PRINCIPAL_DE_LA_MME', label: 'Causa principal MME', type: 'text' },
      { key: 'NOMBRE_DEL_FUNCIONARIO_AL_QUE_SE_LE_ASIGNO_EL_CASO', label: 'Funcionario asignado caso', type: 'text' },
      { key: 'SE_CONCERTO_PLAN_DE_MEJORA_CON_LA_IPS', label: 'Plan de mejora con IPS', type: 'select', options: ['Si', 'No'] },
      { key: 'EVALUACION_Y_SEGUIMIENTO_AL_PLAN_DE_MEJORA', label: 'Evaluacion plan de mejora', type: 'text' },
    ],
  },
  {
    titulo: 'Aborto, parto y complicaciones adicionales',
    icono: 'M13 10V3L4 14h7v7l9-11h-7z',
    fields: [
      { key: 'TIPO_DE_ABORTO', label: 'Tipo de aborto', type: 'select', options: ['NA', 'Espontaneo', 'Inducido'] },
      { key: 'TOMA_DE_PRUEBAS_ITS_INTRAPARTO', label: 'Toma pruebas ITS intraparto', type: 'date' },
      { key: 'RESULTADO_POSITIVO', label: 'Resultado positivo', type: 'text' },
      { key: 'FECHA_DE_DEFUNCION', label: 'Fecha de defuncion', type: 'date' },
      { key: 'CAUSA_DE_LA_DEFUNCION', label: 'Causa de la defuncion', type: 'text' },
    ],
  },
  {
    titulo: 'Recien nacido - Detalles',
    icono: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z',
    fields: [
      { key: 'REGISTRO_CIVIL_RECIEN_NACIDO_1', label: 'Registro civil RN 1', type: 'text' },
      { key: 'NOMBRE_RECIEN_NACIDO_1', label: 'Nombre RN 1', type: 'text' },
      { key: 'SEXO_RECIEN_NACIDO_1', label: 'Sexo RN 1', type: 'select', options: ['FEMENINO', 'MASCULINO'] },
      { key: 'TOMA_TSH_RECIEN_NACIDO_1', label: 'Fecha toma TSH RN 1', type: 'date' },
      { key: 'TOMA_HEMOCLASIFICACION_RECIEN_NACIDO_1', label: 'Fecha hemoclasificacion RN 1', type: 'date' },
      { key: 'DX_HIPOTIROIDISMO', label: 'DX Hipotiroidismo', type: 'text' },
      { key: 'TTO_HIPOTIROIDISMO', label: 'TTO Hipotiroidismo', type: 'text' },
      { key: 'TIEMPO_DE_LECTURA', label: 'Tiempo de lectura', type: 'text' },
      { key: 'UCI_NEONATAL_RECIEN_NACIDO_1', label: 'UCI neonatal RN 1', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'REGISTRO_CIVIL_RECIEN_NACIDO_2', label: 'Registro civil RN 2', type: 'text' },
      { key: 'NOMBRE_RECIEN_NACIDO_2', label: 'Nombre RN 2', type: 'text' },
      { key: 'SEXO_RECIEN_NACIDO_2', label: 'Sexo RN 2', type: 'select', options: ['FEMENINO', 'MASCULINO'] },
      { key: 'PESO_AL_NACER_RECIEN_NACIDO_2', label: 'Peso al nacer RN 2 (g)', type: 'text' },
      { key: 'CONDICION_DEL_RECIEN_NACIDO_2', label: 'Condicion RN 2', type: 'text' },
      { key: 'TOMA_TSH_RECIEN_NACIDO_2', label: 'Fecha toma TSH RN 2', type: 'date' },
      { key: 'TOMA_HEMOCLASIFICACION_RECIEN_NACIDO_2', label: 'Fecha hemoclasificacion RN 2', type: 'date' },
      { key: 'DX_HIPOTIROIDISMO_2', label: 'DX Hipotiroidismo 2', type: 'text' },
      { key: 'TIEMPO_DE_LECTURA_RECIEN_NACIDO_2', label: 'Tiempo de lectura RN 2', type: 'text' },
      { key: 'TTO_HIPOTIROIDISMO_2', label: 'TTO Hipotiroidismo 2', type: 'text' },
      { key: 'UCI_NEONATAL_RECIEN_NACIDO_2', label: 'UCI neonatal RN 2', type: 'select', options: ['NA', 'Si', 'No'] },
      { key: 'VACUNACION_CON_BCG_2', label: 'Vacunacion BCG RN 2', type: 'text' },
      { key: 'VACUNACION_ANTIHEPATITIS_B_2', label: 'Vacunacion antihepatitis B RN 2', type: 'text' },
    ],
  },
  {
    titulo: 'Otros',
    ...
    fields: [
      ...
      { key: 'OBSEVACIONES', label: 'Observaciones', type: 'textarea' },
