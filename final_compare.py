#!/usr/bin/env python
"""Final clean comparison: RAW_FIELDS vs GESTANTE_COLUMNS using _norm() from main.py"""
import unicodedata
...
for csv_name, ftype in RAW_FIELDS:
    ...
    if csv_norm in norm_to_db:
        ...
        extras.append(csv_name)

print(f"Auto-matched (identical names): {auto_ok}")
print(f"Need override (norm matches, names differ): {len(override)}")
print(f"Extra CSV headers (no DB match): {len(extras)}")
print()

print("EXTRA CSV HEADERS (no DB column):")
for e in extras:
    print(f"  {e!r}")
...
for csv_name, db_col in override.items():
    print(f'    {csv_name!r:80s} {db_col!r},')
print("}")
