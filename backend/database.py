...
class HistoriaClinicaAudit(Base):
    __tablename__ = "historias_clinicas_audit"

    id = Column(Integer, primary_key=True)
    historia_id = Column(Integer, nullable=False, index=True)
    ips_name = Column(String(255), nullable=True)
    paciente_documento = Column(String(60), nullable=True)
    paciente_nombre = Column(String(255), nullable=True)
...
def init_db():
    ...
    try:
        ...
        pass
    # Migracion: ampliar contrasena de usuarios_ips para almacenar hashes pbkdf2
    try:
        with engine.begin() as conn:
            if str(engine.url).startswith("postgresql"):
                conn.execute(text("ALTER TABLE usuarios_ips ALTER COLUMN contrasena TYPE VARCHAR(255)"))
    except Exception:
        pass
    ...
    try:
        ...
        pass
    # Migracion: agregar hash_original y hash_actual a historias_clinicas
    try:
        with engine.begin() as conn:
            conn.execute(text("ALTER TABLE historias_clinicas ADD COLUMN hash_original VARCHAR(64)"))
    except Exception:
        pass
    try:
        with engine.begin() as conn:
            conn.execute(text("ALTER TABLE historias_clinicas ADD COLUMN hash_actual VARCHAR(64)"))
    except Exception:
        pass
    # Crear tabla de auditoria de historias clinicas si no existe
    try:
        with engine.begin() as conn:
            is_pg = str(engine.url).startswith("postgresql")
            if is_pg:
                conn.execute(text("""
                    CREATE TABLE IF NOT EXISTS historias_clinicas_audit (
                        id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
                        historia_id INTEGER NOT NULL,
                        ips_name VARCHAR(255),
                        paciente_documento VARCHAR(60),
    ...
    # Crear tablas de reportes de pendientes
    try:
        with engine.begin() as conn:
            is_pg = str(engine.url).startswith("postgresql")
            if is_pg:
                for tbl in ["reporte_consultas", "reporte_medicamentos"]:
                    try:
...
class ReporteConsulta(Base):
    __tablename__ = "reporte_consultas"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, nullable=True)
    ips_name = Column(String(255), nullable=True, index=True)
    consecutivo = Column(Integer, nullable=True)
    periodo_reportado = Column(String(20), nullable=True)
    cod_eps = Column(String(60), nullable=True)
    tipo_documento = Column(String(20), nullable=True)
    documento = Column(String(60), nullable=True)
    identificador_orden = Column(String(120), nullable=True)
    cod_municipio = Column(String(10), nullable=True)
    cod_diagnostico = Column(String(20), nullable=True)
    cups = Column(String(20), nullable=True)
    procedimiento_consulta = Column(String(255), nullable=True)
    clase_pendiente = Column(Integer, nullable=True)
    cantidad_ordenada = Column(Integer, nullable=True)
    cantidad_prestacion_efectiva = Column(Integer, nullable=True)
    cantidad_pendiente = Column(Integer, nullable=True)
    causa_pendiente = Column(Integer, nullable=True)
    observacion_causa = Column(Text, nullable=True)
    fecha_orden = Column(String(20), nullable=True)
    fecha_pendiente = Column(String(20), nullable=True)
    fecha_cierre = Column(String(20), nullable=True)
    patologia = Column(String(120), nullable=True)
    mecanismo_financiacion = Column(String(60), nullable=True)
    tutela = Column(String(5), nullable=True)
    identificacion_prestador = Column(String(120), nullable=True)
...
class ReporteMedicamento(Base):
    __tablename__ = "reporte_medicamentos"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, nullable=True)
    ips_name = Column(String(255), nullable=True, index=True)
    consecutivo = Column(Integer, nullable=True)
    ...
    tipo_documento = Column(String(20), nullable=True)
    documento = Column(String(60), nullable=True)
    identificador_prescripcion = Column(String(120), nullable=True)
    cod_municipio = Column(String(10), nullable=True)
    cod_diagnostico = Column(String(20), nullable=True)
    medicamento_atc = Column(String(60), nullable=True)
    medicamento_concentracion = Column(String(60), nullable=True)
    medicamento_unidad = Column(String(30), nullable=True)
    forma_farmaceutica = Column(String(120), nullable=True)
    medicamento_nombre = Column(String(255), nullable=True)
    mecanismo_financiacion = Column(String(60), nullable=True)
    cantidad_prescrita = Column(Integer, nullable=True)
    dias_tratamiento = Column(Integer, nullable=True)
    cantidad_dispensada = Column(Integer, nullable=True)
    cum_medicamento = Column(String(60), nullable=True)
    cantidad_pendiente = Column(Integer, nullable=True)
    causa_pendiente = Column(Integer, nullable=True)
    observacion_causa = Column(Text, nullable=True)
    fecha_prescripcion = Column(String(20), nullable=True)
    fecha_pendiente = Column(String(20), nullable=True)
    fecha_cierre = Column(String(20), nullable=True)
    cantidad_dispensada_cierre = Column(Integer, nullable=True)
    patologia = Column(String(120), nullable=True)
    identificacion_prestador = Column(String(255), nullable=True)
    tutela = Column(String(5), nullable=True)
    estado = Column(String(20), nullable=False, default="borrador")
    activo = Column(Boolean, nullable=False, default=True)
    created_at = Column(DateTime, nullable=False, default=lambda: datetime.now(timezone.utc))
...
class ReporteAudit(Base):
    __tablename__ = "reportes_audit"
    id = Column(Integer, primary_key=True)
    reporte_id = Column(Integer, nullable=True, index=True)
