#!/usr/bin/env python
"""Complete comparison: find ALL CSV headers needing override mapping."""
import unicodedata
...
# Process every non-FORMULA field
override = {}
no_match = []
...
for csv_name, ftype in RAW_FIELDS:
    ...
    if csv_norm in norm_to_db:
        ...
        if csv_name != db_col:
            override[csv_name] = db_col
    else:
        no_match.append((csv_name, csv_norm))

# Print the override dict
print("CSV_TO_DB_OVERRIDE = {")
for csv_name, db_col in override.items():
    print(f'    "{csv_name}": "{db_col}",')
print("}")
print()
print(f"Override entries: {len(override)}")
print(f"No-match entries: {len(no_match)}")

# Also try to find best-effort matches for no-match entries
print()
print("=" * 100)
print("NO-MATCH ENTRIES (CSV headers with no _norm match in DB):")
print("=" * 100)
for csv_name, csv_norm in no_match:
    print(f"  CSV: {csv_name!r:65s} -> norm: {csv_norm}")
